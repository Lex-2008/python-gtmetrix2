...
def test_requestor_positive(httpserver: HTTPServer):
    ...
    httpserver.expect_oneshot_request("/oneshot").respond_with_json({'value': 42}, status=400)
    ...
    httpserver.expect_oneshot_request("/oneshot").respond_with_json({'errors': [{'status': '404', 'code': 'E404', 'title': 'Not Found'}]})
    with httpserver.wait():
...
def test_requestor_negative(httpserver: HTTPServer):
    ...
    httpserver.expect_oneshot_request("/oneshot").respond_with_json({'value': 42}, status=201)
    with httpserver.wait():
        with pytest.raises(python_gtmetrix2.GTmetrixAPIFailureException, match='invalid status: 201'):
    ...
    with httpserver.wait():
        with pytest.raises(python_gtmetrix2.GTmetrixAPIFailureException, match='empty list'):
            requestor.request("/oneshot")

    httpserver.expect_oneshot_request("/oneshot").respond_with_json({'errors': [{'status': '404', 'code': 'E404', 'title': 'Not Found'},{}]})
    with httpserver.wait():
        with pytest.raises(python_gtmetrix2.GTmetrixAPIFailureException, match='non-error'):
            requestor.request("/oneshot")
...
def test_requestor_retry(httpserver: HTTPServer):
    ...
    httpserver.expect_ordered_request("/ordered").respond_with_json({'errors': [{'status': '429', 'code': 'E42900', 'title': 'Wait 3 sec'}]})
    ...
    httpserver.expect_ordered_request("/ordered").respond_with_json({'errors': [{'status': '429', 'code': 'E42901', 'title': 'Wait 1 sec'}]}, headers={'X-RateLimit-Reset': '1'})
    ...
    httpserver.expect_oneshot_request("/oneshot").respond_with_json({'errors': [{'status': '429', 'code': 'E42902', 'title': 'unknown code'}]})
    with httpserver.wait():
        with pytest.raises(python_gtmetrix2.GTmetrixAPIErrorException):
            requestor.request("/oneshot")
...
def test_test_fetch(httpserver: HTTPServer):
    ...
    httpserver.expect_oneshot_request("/tests/a").respond_with_json(test_json, status=200, headers={'Retry-After':'1'})
    with httpserver.wait():
        test.fetch()
    ...
    httpserver.expect_ordered_request("/tests/a").respond_with_json(test_json, status=303, headers={'Location':'/reports/b'})
    with httpserver.wait():
        test.fetch()
...
def test_test_fetch_wait(httpserver: HTTPServer):
    ...
    test_json['data']['attributes']['n'] = 2
    httpserver.expect_ordered_request("/tests/a").respond_with_json(test_json, status=200, headers={'Retry-After':'1'})
    httpserver.expect_ordered_request("/sleep1").respond_with_data('')
    test_json['data']['attributes']['n'] = 3
    httpserver.expect_ordered_request("/tests/a").respond_with_json(test_json, status=303, headers={'Location':'/reports/b'})
    with httpserver.wait():
...
def test_test_getreport(httpserver: HTTPServer):
    ...
    test_json = {'type':'test','id':'a','attributes':{},'links':{'report':httpserver.url_for("report1")}}
...
def test_interface_start_positive(httpserver: HTTPServer):
    ...
    httpserver.expect_oneshot_request("/tests",method='POST',
            headers={'Content-Type':'application/vnd.api+json'},
            json={'data':{'type':'test','attributes':{'url':'example.com','key':'value'}}}
            ).respond_with_json(
                    {'data':{'type':'test','id':'a','attributes':{"source":"api","state":"queued",'key':'value'}}},
                    status=202,
...
def test_interface_list_positive(httpserver: HTTPServer):
    interface = python_gtmetrix2.Interface('aaa', httpserver.url_for(""))
    httpserver.expect_oneshot_request("/tests").respond_with_json({'data':[{'type':'test','id':'a','attributes':{}}]})
    with httpserver.wait():
