    ...
    def _plain_request(self, url, **kwargs):
        ...
        if __debug__:
            ...
            if 'valid_statuses' in kwargs and response.status not in kwargs['valid_statuses']:
                raise GTmetrixAPIFailureException(('API returned invalid status: %s' % response.status),
                        request, response, response.read())
        data = response.read()
        if len(data) == 0:
        ...
        if 'errors' in json_data:
            if __debug__:
                ...
                if len(json_data['errors']) < 1:
                    raise GTmetrixAPIFailureException('API returned empty list of errors', request, response, json_data)
                if not all( (dict_is_error(x) for x in json_data['errors']) ):
                    raise GTmetrixAPIFailureException('API returned non-error in error list', request, response, json_data)
    ...
    def fetch(self, wait_for_complete=False, retries=10):
        ...
        if __debug__:
            if not 'data' in response_data:
                raise GTmetrixAPIFailureException('API returned no data for a test', None, response, response_data, self)
            if not dict_is_test(response_data['data']):
                raise GTmetrixAPIFailureException('API returned non-test for a test', None, response, response_data, self)
    ...
    def fromURL(cls, requestor, url, sleep_function=time.sleep):
        ...
        if __debug__:
            ...
            if not dict_is_report(response_data['data']):
                raise GTmetrixAPIFailureException('API returned non-report for a report', None, response, response_data)
...
class Interface:
    ...
    def start_test(self, url, **attributes):
        ...
        (response, response_data) = self.requestor.request('tests', {'data':data}, method='POST', headers={'Content-Type':'application/vnd.api+json'})
        if __debug__:
            if not 'data' in response_data:
                raise GTmetrixAPIFailureException('API returned no data for a started test', None, response, response_data)
            if not dict_is_test(response_data['data']):
                raise GTmetrixAPIFailureException('API returned non-test for a started test', None, response, response_data)
    ...
    def list_tests(self):
        ...
        if __debug__:
            if not 'data' in response_data:
                raise GTmetrixAPIFailureException('API returned no data for a list of tests', None, response, response_data)
            if not isinstance(response_data['data'], list):
                raise GTmetrixAPIFailureException('API returned non-list for a list of tests', None, response, response_data)
            if not all((dict_is_test(test) for test in response_data['data'])):
                raise GTmetrixAPIFailureException('API returned non-test in a list of tests', None, response, response_data)
